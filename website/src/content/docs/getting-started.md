...
By default the daemon stores app configs, logs, and runtime state under
`~/.harbor` (override with the `HARBOR_HOME` environment variable) and
binds its management API to `127.0.0.1:4780`. On first run it writes a
random bearer token to `~/.harbor/token`, which the CLI reads
automatically — you don't need to handle it yourself.
